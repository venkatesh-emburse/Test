...
export class GetNearbyUsersQueryDto {
    ...
    @ApiPropertyOptional({ example: 10, description: 'Radius in kilometers (max 50)', default: 10 })
    @IsOptional()
    @IsNumber()
    @Min(1)
    @Max(50)
    ...
    @ApiPropertyOptional({ example: 20, description: 'Max users to return', default: 20 })
    @IsOptional()
    @IsNumber()
    @Min(1)
    @Max(50)
    ...
    @ApiPropertyOptional({ description: 'Only show verified users', default: false })
    @IsOptional()
    @IsBoolean()
    @Type(() => Boolean)
...
export class NearbyUserDto {
    ...
    @ApiPropertyOptional({ description: 'Approximate location (privacy-protected)' })
    approximateLocation?: {
        latitude: number;
        longitude: number;
    };
