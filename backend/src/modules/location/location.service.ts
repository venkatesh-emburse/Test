...
export class LocationService {
    ...
    async getNearbyUsers(
        ...
        const { latitude, longitude, radiusKm = 10, limit = 20, verifiedOnly = false } = query;
        ...
        const excludeIds = [userId, ...swipedUserIds, ...matchedUserIds, ...blockedUserIds];
        ...
        if (verifiedOnly) {
            queryBuilder = queryBuilder.andWhere('"user"."is_verified" = :verified', { verified: true });
        }
        ...
        return usersWithDistance.entities.map((user, index) => {
            ...
            return this.mapToNearbyUserDto(user, distanceKm, latitude, longitude);
        });
    }
    ...
    async getUsersInBounds(
        ...
        const matchedUserIds = await this.getMatchedUserIds(userId);
        const blockedUserIds = await this.getBlockedUserIds(userId);
        const excludeIds = new Set([userId, ...swipedUserIds, ...matchedUserIds, ...blockedUserIds]);
        ...
        return usersWithDistance.entities.map((user, index) => {
            ...
            return this.mapToNearbyUserDto(user, distanceKm, centerLat, centerLng);
        });
    }
    ...
    private mapToNearbyUserDto(
        ...
        // Add random offset to location for privacy (up to 500m)
        ...
        return {
            ...
            approximateLocation: user.showOnMap
                ? {
                    latitude: refLat + latOffset,
                    longitude: refLng + lngOffset,
                }
                : undefined,
