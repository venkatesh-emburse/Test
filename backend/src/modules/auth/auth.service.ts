...
export class AuthService {
  ...
  async verifyPhoneOtp(dto: VerifyPhoneOtpDto) {
    ...
    return this.verifyOtpAndAuthenticate(
      phone,
      code,
  ...
  async generateTokens(user: User) {
    ...
    return {
      ...
      tokenType: 'Bearer',
      expiresIn:
        this.parseExpiresIn(this.configService.get('jwt.expiresIn') ?? '7d') /
        1000,
  ...
  async setIntent(userId: string, dto: SetIntentDto) {
    ...
    // Allow changing intent anytime, but frequent changes will reduce
    // trust score (handled by safety score recalculation on the client side).
    ...
    return {
      success: true,
      intent: user.intent,
      intentChangedAt: user.intentChangedAt,
    };
  }
  ...
  async createProfile(userId: string, dto: CreateProfileDto) {
    ...
    profile.profileCompleteness = this.calculateProfileCompleteness(
      user,
      profile,
    );
