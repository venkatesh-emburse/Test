import { IsString, IsNotEmpty, IsOptional, IsEnum, Matches, IsEmail, MinLength, MaxLength } from 'class-validator';
...
export class CreateProfileDto {
    ...
    @ApiProperty({ example: '1995-06-15', description: 'Date of birth in YYYY-MM-DD format' })
    @IsString()
    @IsNotEmpty()
    ...
    @ApiPropertyOptional({ example: 'Software engineer who loves travel and music.' })
    @IsString()
    @IsOptional()
    @MaxLength(500)
    bio?: string;

    @ApiPropertyOptional({ example: 'Looking for someone who shares my passion for adventure.' })
    @IsString()
    @IsOptional()
    @MaxLength(300)
    lookingFor?: string;
...
export class AuthTokensResponseDto {
    ...
    @ApiProperty({ example: 604800, description: 'Access token expiry in seconds' })
    expiresIn: number;
